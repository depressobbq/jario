...
public class Background
{
    ...
    public int get_tile(int hoffset, int voffset)
    {
        ...
        return (self.vram[tiledata_addr + 0] << 0) + (self.vram[tiledata_addr + 1] << 8);
    }
    ...
    public void offset_per_tile(int x, int y, int[] hoffset, int[] voffset)
    {
        ...
        if (opt_x >= 8)
        {
            hval = self.bg3.get_tile((int)((opt_x - 8) + (self.bg3.regs.hoffset & ~7)), self.bg3.regs.voffset + 0);
            if (self.regs.bgmode != 4)
            {
                vval = self.bg3.get_tile((int)((opt_x - 8) + (self.bg3.regs.hoffset & ~7)), self.bg3.regs.voffset + 8);
            }
    ...
    public void render()
    {
        ...
        int bgpal_index = (self.regs.bgmode == 0 ? id << 5 : 0);
        ...
        boolean is_opt_mode = (self.regs.bgmode == 2 || self.regs.bgmode == 4 || self.regs.bgmode == 6);
        boolean is_direct_color_mode = (self.screen.regs.direct_color == true && id == ID_BG1 && (self.regs.bgmode == 3 || self.regs.bgmode == 4));
        ...
        while (x < width)
        {
            ...
            ByteBuffer tiledata = self.cache.tile(regs.mode, tile_num);
            ...
            for (int n = 0; n < 8; n++, x++)
            {
                ...
                if (hires == false)
                {
                    ...
                    {
                        self.screen.output.plot_main(x, mosaic_color, mosaic_priority, id);
                    }
                    if (regs.sub_enable && (window.sub[x] == 0))
                    {
                        self.screen.output.plot_sub(x, mosaic_color, mosaic_priority, id);
                    }
                }
                else
                {
                    ...
                    if ((x & 1) != 0)
                    {
                        ...
                        {
                            self.screen.output.plot_main(half_x, mosaic_color, mosaic_priority, id);
                        }
                    }
                    else
                    {
                        ...
                        {
                            self.screen.output.plot_sub(half_x, mosaic_color, mosaic_priority, id);
                        }
                    }
                }
            }
    ...
    public void render_mode7()
    {
        ...
        for (int x = 0; x < 256; x++)
        {
                ...
                case 1:
                    {
                        ...
                        tx = ((px >> 3) & 127);
                        ty = ((py >> 3) & 127);
                        tile = self.vram[(ty * 128 + tx) << 1];
                        palette = self.vram[(((tile << 6) + ((py & 7) << 3) + (px & 7)) << 1) + 1];
                        break;
                    }
            ...
            int plot_x = (self.regs.mode7_hflip == false ? x : 255 - x);

            int color;
            if (self.screen.regs.direct_color && id == ID_BG1)
            {
