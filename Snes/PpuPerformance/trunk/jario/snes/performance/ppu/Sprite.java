...
public class Sprite
{
    ...
    {
        self.regs.oam_addr = (self.regs.oam_baseaddr << 1) & 0xFFFF;
        set_first();
    }
    ...
    public void render()
    {
        ...
        for (int i = 0; i < 34; i++)
        {
            ...
            ByteBuffer tiledata = self.cache.tile_4bpp(t.tile);
        ...
        if (priority_table[0] + priority_table[1] + priority_table[2] + priority_table[3] == 0)
        {
            return;
        }
        ...
        for (int x = 0; x < 256; x++)
        {
            ...
            int color = self.screen.get_palette(output.palette[x]);
            if (regs.main_enable && (window.main[x] == 0))
            {
                self.screen.output.plot_main(x, color, priority, 4 + ((palette < 192) ? 1 : 0));
            }
            if (regs.sub_enable && (window.sub[x] == 0))
            {
                self.screen.output.plot_sub(x, color, priority, 4 + ((palette < 192) ? 1 : 0));
            }
        }
    }
    ...
    public Sprite(PPU self)
    {
        ...
        for (int i = 0; i < tilelist.length; i++) tilelist[i] = new TileList();
    }
