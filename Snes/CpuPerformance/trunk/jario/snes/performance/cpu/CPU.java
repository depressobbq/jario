...
//            chip.Processor.clock -= (long)(clocks * (long)chip.Processor.frequency);
//        }
    }
...
//    }
    ...
    private Callback queue_event = new Callback() {
    ...
    final void add_clocks(int clocks)
    {
        if (status.hirq_enabled)
        {
            if (status.virq_enabled)
            {
                ...
                //int framelines = (System.system.region == System.Region.NTSC ? 262 : 312) + ((PPUCounter.read32bit(2) != 0) ? 1 : 0);
...
//        {
//            queue.enqueue((input_port.read8bit(-1)&0xFF), QueueEvent_ControllerLatch);
//        }
